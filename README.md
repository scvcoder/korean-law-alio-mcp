...
"○○진흥원 인사규정 상위법"           → cross-domain (ALIO → 법제처)
"근로기준법 따르는 공공기관 규정"            → cross-domain (역방향)
...
> ⚠️ **데이터 mirror 갱신은 best-effort** — Releases 의 `tag` 가 수집 일자, 압축 안 manifest.json `fetchedAt` 으로 정확한 시점 확인. 시점이 중요한 사용에는 `npm run alio:sync` 로 직접 최신화 권장. 갱신 차이로 발생하는 문제는 사용자 책임. 자세한 책임 분담은 [`NOTICE`](./NOTICE) 참고.
...
node build/cli.js "○○진흥원 인사규정 상위법"        # cross-domain (ALIO → 법제처)
node build/cli.js "근로기준법 따르는 공공기관 규정"         # cross-domain (역방향)
...
### Cross-domain 브리지 (fork 신규 — ALIO ↔ 법제처)
...
23개 ALIO 도구 + 3개 cross-domain 브리지 전체는 [`docs/API.md`](./docs/API.md) 또는 [`ROADMAP.md`](./ROADMAP.md) 참고.
